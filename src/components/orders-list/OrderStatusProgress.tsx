...
import {selectOrdersAges, selectOrdersStatus} from "@/ducks/orders/openOrdersSlice.ts";
...
const OrderStatusProgress = () => {
    ...
    return (
        <ProgressBar style={{minWidth: '20rem', width: '100%', height: '1rem'}}>
            {status === 'loading' && <ProgressBar striped animated now={100} label="Loading..."/>}
            {status === 'idle' && Object.keys(ages).map(age => (
